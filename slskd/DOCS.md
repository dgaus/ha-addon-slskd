...
2. Point **Shared directories** at the folders you want to share (default `/media/music`).
   ...
   refused by others.
3. Start the app and open the UI from the sidebar (**slskd**).
...
in the app's Network section (and your router rule) to match.
...
- Note that with ingress enabled, slskd serves under the ingress URL path. Direct
  requests must use the same base path (shown in the app log at startup as
  `Serving under ingress path: ...`). For API clients, an API key configured in
  slskd's options file (`/addon_configs/..._slskd/slskd.yml`) is the cleaner route.

## Advanced configuration

Anything not covered by the app options (upload/download slots, speed limits,
user groups, filters, …) can be set in slskd's own configuration file, which this
app keeps in the app configuration folder so you can edit it from Home
Assistant: open the **File editor** (or **Studio Code Server** / **Samba**) app
...
- Settings managed by this app (Soulseek credentials, shared/download
  directories, listen port, web UI auth) are passed as environment variables, which
  **override** the YAML — change those in the app's Configuration tab, not in
  slskd.yml.
